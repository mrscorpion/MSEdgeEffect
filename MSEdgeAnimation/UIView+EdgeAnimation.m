...
static NSString *const MSDDirectionKey  = @"directionKey";
static NSString *const MSEdgeLayerKey   = @"edgelayerKey";
static NSString *const MSEndPointKey    = @"endpointKey";
static NSString *const MSEdgeColorKey   = @"edgecolorKey";
static NSString *const MSPanGestureKey   = @"panGestureKey";
...
- (void)addEdgeEffect
{
    UIPanGestureRecognizer *panGesture = objc_getAssociatedObject(self, (__bridge const void *)(MSPanGestureKey));
    if (!panGesture) {
        panGesture = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(panGesture:)];
        objc_setAssociatedObject(self, (__bridge const void *)(MSPanGestureKey), panGesture, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        Method mothod1 = class_getInstanceMethod([self class], NSSelectorFromString(@"addGestureRecognizer:"));
        Method mothod2 = class_getInstanceMethod([self class], NSSelectorFromString(@"MSaddGestureRecognizer:"));
...
// 防止手势被覆盖
- (void)MSaddGestureRecognizer:(UIGestureRecognizer *)gestureRecognizer
{
    if ([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        UIPanGestureRecognizer *pan = objc_getAssociatedObject(self, (__bridge const void *)(MSPanGestureKey));
        if (pan && pan!= gestureRecognizer) {
            return;
        }
    }
    objc_msgSend(self, NSSelectorFromString(@"MSaddGestureRecognizer:"),gestureRecognizer);
}
...
{
    CAShapeLayer *layer = objc_getAssociatedObject(self, (__bridge const void *)(MSEdgeLayerKey));
    if (!layer) {
...
{
    objc_setAssociatedObject(self, (__bridge const void *)(MSEdgeLayerKey), edgeLayer, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (EdgeAnimationDirection)direction
{
    NSString *string = objc_getAssociatedObject(self, (__bridge const void *)(MSDDirectionKey));
    return string.intValue;
}

- (void)setDirection:(EdgeAnimationDirection)direction
{
    objc_setAssociatedObject(self, (__bridge const void *)(MSDDirectionKey), [NSString stringWithFormat:@"%ld", (long)direction], OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (CGPoint)endPoint
{
    NSString *pointString = objc_getAssociatedObject(self, (__bridge const void *)(MSEndPointKey));
...
{
    objc_setAssociatedObject(self, (__bridge const void *)(MSEndPointKey), NSStringFromCGPoint(endPoint), OBJC_ASSOCIATION_COPY_NONATOMIC);
}

- (UIColor *)edgeFillColor
{
    UIColor *color = objc_getAssociatedObject(self, (__bridge const void *)(MSEdgeColorKey));
    if (!color) {
...
{
    objc_setAssociatedObject(self, (__bridge const void *)(MSEdgeColorKey), edgeFillColor, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
@end
